...
// A very small software synthesiser. Everything the garden plays is generated
// here as samples rather than shipped as audio files: no assets, no cgo, and
// the whole thing stays testable because nothing in this file touches a
// speaker.
...
func (v *voice) render(buf []float64, pos int64, rate float64) bool {
	...
	for i := range buf {
		...
		buf[i] += v.gain * v.env.at(t) * shape(v.wave, v.phase)
	}
...
func newMixer(rate int) *mixer {
	return &mixer{rate: rate, gain: 0.32}
}
